...
class MovimentacaoDAO {
    ...
    public function saveMovimentacao(Movimentacao $m) {
        $query = "INSERT INTO movimentacao (bebida_id, tipo, volume, responsavel) VALUES (:bebida_id, :tipo, :volume, :responsavel)";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':bebida_id', $m->bebida_id);
        $stmt->bindParam(':tipo', $m->tipo);
        $stmt->bindParam(':volume', $m->volume);
        $stmt->bindParam(':responsavel', $m->responsavel);
        return $stmt->execute();
    }

    public function updateMovimentacao(Movimentacao $h) {
        $query = "UPDATE movimentacao SET bebida_id = :bebida_id, tipo = :tipo, volume = :volume, responsavel = :responsavel, data_registro = now() WHERE id = :id";
        try {
            if (!$this->conn) {
